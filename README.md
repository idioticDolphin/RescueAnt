...
5. **Review/adjust `bot.config`**
   ...
   | `search_provider`, `search_api_key`, `search_engine_id`, ... | search API configuration, only needed if `discover_urls = True` - see "Search engines" below for Google setup and how to use a different provider |
...
### Using Google (the default)

The default `bot.config` uses Google's free
[Programmable Search Engine](https://programmablesearchengine.google.com/)
(Custom Search JSON API, 100 free queries/day):
...
2. Create a search engine at https://programmablesearchengine.google.com/,
   then in its settings switch "Sites to search" to **Search the entire
   web** (by default a new search engine is restricted to specific sites).
3. Copy the API key into `search_api_key` and the search engine's "Search
   engine ID" (cx) into `search_engine_id` in `bot.config`.
...
Any other provider - a self-hosted [SearXNG](https://docs.searxng.org/)
instance, Bing Web Search, SerpApi, etc. - can be plugged in via
`ConfigurableJsonSearchProvider` (see
`model.objects.searchprovider.ConfigurableJsonSearchProvider`'s docstring
for the full picture) by setting `search_provider` to anything other than
`"Google"` and filling in these keys instead of `search_api_key`/`search_engine_id`:
...
database. It needs the model downloaded (step 4 above) and, to see
discovery actually find anything, real Google credentials filled into
`examples/bot.config` (see "Search engines" above). Without them, each
discovery query simply fails (logged, not fatal - see
...
bot.config                 runtime configuration (see step 5 above)
