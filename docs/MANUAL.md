...
| `discovery_when_yield_below` | float | `0.0` | Run discovery once the pages coming back stop being worth extracting: the share of classified pages in an extracting category, over the last `yield_window_rounds` rounds. `0` turns the check off. This is the check that does not depend on the scores - a category the classifier overfills passes its weight to every link on the page, so a frontier can look promising for a whole night while returning nothing. |
| `yield_window_rounds` | int | `5` | How many rounds that share is measured over. The alarm stays quiet until it has that many. |
